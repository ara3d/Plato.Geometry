...
namespace Common.Unity.Drawing
{
    ...
    public class SegmentRenderer : BaseRenderer
    {
        ...
        private void DrawVerticesAsPentagons(Camera camera, Matrix4x4 localToWorld, int[] indices)
        {
            ...
            for (var i = 0; i < indices.Length / 5; i++)
            {
                var i0 = indices[i * 5 + 0];
                var i1 = indices[i * 5 + 1];
                var i2 = indices[i * 5 + 2];
                var i3 = indices[i * 5 + 3];
                var i4 = indices[i * 5 + 4];
        ...
        private void DrawVerticesAsHexagons(Camera camera, Matrix4x4 localToWorld, int[] indices)
        {
            ...
            {
                var i0 = indices[i * 6 + 0];
                var i1 = indices[i * 6 + 1];
                var i2 = indices[i * 6 + 2];
                var i3 = indices[i * 6 + 3];
                var i4 = indices[i * 6 + 4];
                var i5 = indices[i * 6 + 5];
